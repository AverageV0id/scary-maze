...
while run:
    ...
    jumpscare_surf.fill("cyan")
    screen.blit(jumpscare_surf, jumpscare_point_rect)

    me_surf = pg.Surface((10, 10))
    me_surf.fill(me_color)

    me = pg.Rect((x, y, 5, 5))
    screen.blit(me_surf, me)

    me_rect = me_surf.get_rect(topleft=(x, y))
    ...
    text = font.render(f"Level 3", False, 'white')
